...
public class ClientGameManager implements Runnable {
                ...
                if (Game.getMove().isAttackMove() == true) {
                    ...
                    if (attackingPiece.getPieceType() == PieceType.SCOUT) {
                        ...
                        if (Math.abs(moveX) > 1 || Math.abs(moveY) > 1) {
                            ShiftXY shiftXY = shiftXY(moveX, moveY);
                            Platform.runLater(() -> {
                                try {


                                    // Move the scout in front of the piece it's attacking before actually fading out
                                    ClientSquare scoutSquare = Game.getBoard().getSquare(Game.getMove().getEnd().x + shiftXY.getShiftX(), Game.getMove().getEnd().y + shiftXY.getShiftY());
                            ...
                            Game.getBoard().getSquare(Game.getMove().getEnd().x + shiftXY.getShiftX(), Game.getMove().getEnd().y + shiftXY.getShiftY()).setPiece(startSquare.getPiece());
                            Game.getBoard().getSquare(Game.getMove().getStart().x, Game.getMove().getStart().y).setPiece(null);

                            Game.getMove().setStart(Game.getMove().getEnd().x + shiftXY.getShiftX(), Game.getMove().getEnd().y + shiftXY.getShiftY());
                        }
                    }
                    Platform.runLater(() -> {
                        try {
    ...
    public ShiftXY shiftXY (int moveX, int moveY){
        ShiftXY shiftXY = new ShiftXY(0,0);
